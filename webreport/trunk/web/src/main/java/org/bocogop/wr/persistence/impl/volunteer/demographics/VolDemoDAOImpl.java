package org.bocogop.wr.persistence.impl.volunteer.demographics;
...
import static org.bocogop.wr.persistence.dao.volunteer.demographics.VolDemoColumn.ACTIVE_ASSIGNMENTS;
import static org.bocogop.wr.persistence.dao.volunteer.demographics.VolDemoColumn.PARKING_STICKERS;
import static org.bocogop.wr.persistence.dao.volunteer.demographics.VolDemoColumn.SUPERVISORS;
import static org.bocogop.wr.persistence.dao.volunteer.demographics.VolDemoColumn.TOTAL_DONATIONS;
import static org.bocogop.wr.persistence.dao.volunteer.demographics.VolDemoColumn.UNIFORMS;
...
public class VolDemoDAOImpl extends AbstractAppDAOImpl<VolunteerDemographics> implements VolDemoDAO {
	...
	public List<VolunteerDemographics> findDemographics(VolDemoSearchParams searchParams, int start, int length) {
		...
		if (searchParams.isLocal()) {
			sc.append("			vols_at_facility AS (") //
					.append("		SELECT DISTINCT TOP 2000000000 va.WrVolunteersFK") //
					.append("		FROM wr.VolunteerAssignments va") //
		...
		sc.append("				all_assigned_vols AS (") //
				.append("			SELECT *") //
				.append("			FROM wr.Volunteers v");
		if (searchParams.isLocal())
			sc.append("					LEFT JOIN vols_at_facility vaf ON v.id = vaf.WrVolunteersFK");
		sc.append("					WHERE 1=1"); //
		...
		sc.append("	FROM all_assigned_vols v") //
				...
				.append("		JOIN wr.WR_STD_VolunteerStatus vs ON v.WR_STD_VolunteerStatusFK = vs.id") //
				...
				.append("		LEFT JOIN wr.SUMM_Volunteer_Hours svh on v.id = svh.VolunteerId"); //
		if (includeAssignments)
	...
	private void appendCTEForAssignments(SearchContext sb) {
		sb.append("				,vol_assignments AS (") //
				...
				.append("					WHERE va.WrVolunteersFK = v.Id") //
	...
	public void includeCTEForUniforms(SearchContext sb) {
		sb.append("				,vol_uniforms AS (") //
				...
				.append("					WHERE u.WrVolunteersFK = v.Id"); //
	...
	public void appendCTEForParkingStickers(SearchContext sb) {
		sb.append("				,vol_parking_stickers AS (") //
				...
				.append("					WHERE ps.WrVolunteersFK = v.Id"); //
		if (sb.searchParams.isLocal())
	...
	private void appendWhereClauseItemsForVolunteerRestrictions(SearchContext sb) {
		...
		if (sb.searchParams.isLocal()) {
			sb.append("				AND (") //
					...
					.append("			OR vaf.WrVolunteersFK IS NOT NULL)"); //
		}
		...
		sb.append("	AND v.WR_STD_VolunteerStatusFK in (-1");
